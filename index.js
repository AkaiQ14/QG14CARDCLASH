...
const IS_DESKTOP = String(process.env.QG14_DESKTOP || "").toLowerCase() === "1";
const PUBLIC_ROOT = process.env.QG14_PUBLIC_ROOT
  ? path.resolve(process.env.QG14_PUBLIC_ROOT)
  : path.join(__dirname, "public");
const CARD_LIBRARY_ROOT = process.env.QG14_CARD_LIBRARY_ROOT
  ? path.resolve(process.env.QG14_CARD_LIBRARY_ROOT)
  : "";
...
function getCardAssets(scope) {
  ...
  // Desktop mode keeps the heavy card library outside the packaged app.
  // The external library mirrors the existing project layout:
  //   <library>/public/images/{normal,legendary,fullscreen}
  //   <library>/public/anime/images/{normal,legendary,fullscreen}
  // This also lets the desktop app point directly at an existing project root.
  const imageRoot = CARD_LIBRARY_ROOT
    ? path.join(
        CARD_LIBRARY_ROOT,
        "public",
        ...(cardScope === "anime" ? ["anime"] : []),
        "images"
      )
    : path.join(PUBLIC_ROOT, scopeDirectory, "images");
...
// Desktop runtime information. No secrets are returned here.
app.get("/api/desktop-info", (req, res) => {
  res.json({
    desktop: IS_DESKTOP,
    cardLibraryExternal: !!CARD_LIBRARY_ROOT,
  });
});



// Serve heavy desktop card folders from the external card library first.
// Only normal / legendary / fullscreen are externalized; UI assets, sounds,
// logos and other files continue to come from PUBLIC_ROOT.
if (CARD_LIBRARY_ROOT) {
  const externalMounts = [
    ["/images/normal", path.join(CARD_LIBRARY_ROOT, "public", "images", "normal")],
    ["/images/legendary", path.join(CARD_LIBRARY_ROOT, "public", "images", "legendary")],
    ["/images/fullscreen", path.join(CARD_LIBRARY_ROOT, "public", "images", "fullscreen")],
    ["/anime/images/normal", path.join(CARD_LIBRARY_ROOT, "public", "anime", "images", "normal")],
    ["/anime/images/legendary", path.join(CARD_LIBRARY_ROOT, "public", "anime", "images", "legendary")],
    ["/anime/images/fullscreen", path.join(CARD_LIBRARY_ROOT, "public", "anime", "images", "fullscreen")],
  ];

  for (const [urlPath, diskPath] of externalMounts) {
    try { fs.mkdirSync(diskPath, { recursive: true }); } catch {}
    app.use(urlPath, express.static(diskPath, { fallthrough: true }));
  }
}

// Serve packaged/static UI files AFTER ipAllowlist and card-library mounts.
...
function logServerStartup() {
  ...
  if (HOST) console.log(`[server] host=${HOST}`);
  if (IS_DESKTOP) {
    console.log("[desktop] QG14 desktop mode enabled");
    console.log(`[desktop] public root: ${PUBLIC_ROOT}`);
    console.log(`[desktop] card library: ${CARD_LIBRARY_ROOT || "bundled public/images"}`);
  }
